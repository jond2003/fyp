...
from global_vars import BLOGS_TEST_PATH, BERT_PATH, TEXT_GEN_PATH
...
# test classifier accuracy using test blogs
def test_classifier():
    print("\nUsing test data to validate BERT classifier...")
    # load training data
    df = pd.read_csv(BLOGS_TEST_PATH)
    ...
    for i in range(len(blogs)):
        blog = blogs[i]
        exp_blogger = labels[i]
        pred_blogger = predict_blogger(blog, model, tokenizer, device)

        if exp_blogger == pred_blogger:
            correct_preds += 1
    
    print(f"Predicted blogs with {(correct_preds*100)/len(blogs):.2f}% accuracy")
